...
Scene loadSceneFromFile(std::istream &in) {
    ...
    for (auto &figure: scene.figures) {
        if (figure->emission.r > 0 || figure->emission.g > 0 || figure->emission.b > 0) {
            if (figure->type == Figures::BOX) {
                lights.push_back(new BoxLight(rnd, *figure));
            } else if (figure->type == Figures::ELLIPSOID) {
                lights.push_back(new EllipsoidLight(rnd, *figure));
            }
        }
    }
    std::vector<Distribution*> mix;
    mix.push_back(new Cosine(rnd));
    if (!lights.empty()) {
        auto light = new Mix(rnd, std::move(lights));
        mix.push_back(light);
    }
    scene.distribution = new Mix(rnd, std::move(mix));
...
Color Scene::getPixelColor(Ray ray, int bounceNum) const {
    ...
    if (intersectedObject->material == Material::METALLIC || intersectedObject->material == Material::DIELECTRIC) {
        ...
        Point w = distribution->sample(p + 0.0001 * normal, normal);
        if (w * normal < 0) {
        ...
        auto rec_color = 1.0 / (acos(-1) * pdf) * (w * normal) * intersectedObject->color * getPixelColor(wR, bounceNum - 1);
        return {intersectedObject->emission.r + rec_color.r, intersectedObject->emission.g + rec_color.g,
                intersectedObject->emission.b + rec_color.b};
    }
}
